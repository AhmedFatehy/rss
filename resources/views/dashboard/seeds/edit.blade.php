...
@section('title')
    {{ trans('seeds.name') }} : {{ trans('app.update') }} : {{ $data['title'] }}
@endsection
...
@section('content')
    <div class="portlet box green">
        <div class="portlet-title">
            <div class="caption">
                <i class="fa fa-gift"></i> {{ trans('categories.edit.form_caption') }} | {{ $data['title'] }} </div>
            <div class="tools">
        ...
        <div class="portlet-body form">
            <!-- BEGIN FORM-->
            {!! Form::model($data, ['route' => ['seeds.update',$data['slug']], 'method' => 'PATCH', 'class' => 'form-horizontal']) !!}
            {!! csrf_field()!!}
            <div class="form-body">
                <div class="form-group">
                    <label class="col-md-3 control-label"> {{ trans('seeds.label.title') }}</label>
                    <div class="col-md-4">
                        <input type="text" name="title" class="form-control input-circle"  value="{{$data['title']}}" placeholder="اسم المصدر" required>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-md-4">
                        <input type="url" name="url" class="form-control input-circle"
                               value="{{$data['url']}}" placeholder="رابط المصدر" required>
                    </div>
                </div>
                                ...
                                <select id="single" name="category_id" class="form-control select2  input-circle" required>
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category['id'] }}"
                                                @if($data['category_id'] == $category['id']) selected @endif>{{ $category['title'] }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-9">
                        <textarea name="description" id="summernote_1" required
                                  class="input-circle">{{$data['description']}}</textarea>
                    </div>
                </div>
                <div class="form-group">
                ...
                <div class="form-group last">
                    ...
                    <div class="col-md-4">
                        <input type="checkbox" name="status" @if($data['status']) checked
            ...
            <div class="form-actions">
                <div class="row">
                    <div class="col-md-offset-3 col-md-9">
                        <button type="submit"
                                class="btn btn-circle green"> {{ trans('seeds.edit.submit') }} </button>
                        <button type="button"
