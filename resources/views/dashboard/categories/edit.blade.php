...
@section('title')
    {{ trans('categories.name') }} : {{ trans('app.update') }} : {{ $data['title'] }}
@endsection
...
@section('content')
    <div class="portlet box green">
        ...
        <div class="portlet-body form">
            <!-- BEGIN FORM-->
            {!! Form::model($data, ['route' => ['categories.update',$data['slug']], 'method' => 'PATCH', 'class' => 'form-horizontal']) !!}
            {!! csrf_field()!!}
            <div class="form-body">
                <div class="form-group">
                    <label class="col-md-3 control-label"> {{ trans('categories.label.title') }}</label>
                    <div class="col-md-4">
                        <input type="text" name="title" class="form-control input-circle" value="{{$data['title']}}"
                                ...
                                <select id="single" name="parent_id" class="form-control select2  input-circle">
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category['id'] }}"
                                                @if($data['parent_id'] == $category['id']) selected @endif>{{ $category['title'] }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
            ...
            <div class="form-actions">
                <div class="row">
                    <div class="col-md-offset-3 col-md-9">
                        <button type="submit"
                                class="btn btn-circle green"> {{ trans('categories.edit.submit') }} </button>
                        <button type="button"
